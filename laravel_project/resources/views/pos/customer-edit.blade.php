    ...
    <div class="main-content">
        ...
        <div class="flex items-center flex-wrap justify-between gap20 mb-27">
            ...
            <ul class="breadcrumbs flex items-center flex-wrap justify-start gap10">
                ...
                <li>
                    <a href="{{route('pos.customers')}}"><div class="text-tiny">customers</div></a>
                </li>
                <li>
                    <i class="icon-chevron-right"></i>
                </li>
        ...
        <!-- new-category -->
        <div class="wg-box">
            <form class="form-new-product form-style-1" action="{{route('pos.customer-update')}}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="hidden" name="id" value="{{$customer->id}}" />
                 <fieldset class="name">
                            ...
                            <input class="flex-grow" type="text" placeholder="customer name" name="name"
                                tabindex="0" value="{{ $customer->name }}" aria-required="true">
                        </fieldset>
                        @error('name')
                        ...
                        <fieldset class="name">
                            ...
                            <input class="flex-grow" type="text" placeholder="phone number" name="phone"
                                tabindex="0" value="{{ $customer->phone }}" aria-required="true">
                        </fieldset>
                        @error('phone')
                            <span class="alert alert-danger text-center">{{ $message }}</span>
                        @enderror
                        <fieldset class="name">
                            ...
                            <input class="flex-grow" type="text" placeholder="city" name="city"
                                tabindex="0" value="{{ $customer->city }}" aria-required="true">
                        </fieldset>
                        @error('city')
                            <span class="alert alert-danger text-center">{{ $message }}</span>
                        @enderror <fieldset class="name">
                            <div class="body-title">Address <span class="tf-color-1">*</span></div>
                           <textarea class="flex-grow" placeholder="address" name="address" tabindex="0" aria-required="true">{{ $customer->address }}</textarea>
